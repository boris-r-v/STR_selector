...
def change_outliers_to_median_into_series ( series: pd.Series ):
    ...
    ns = series.copy()
    ns = ns.transform( lambda x: x if ( not math.isnan(x) ) else median )
...
def draw_plot(dfs, titles, logger ):
    ...
    print (dfs[0].to_csv("0.csv", index=False, header=titles))
    print (dfs[1].to_csv("1.csv", index=False, header=titles))
